...
def local_connect():
    """Logs locally registered users in."""
    # Get form data.
    ...
    if user is not None and user.verify_password(password):
        ...
        login_session["access_token"] = user.generate_auth_token(
            login_session["state"])
...
def fbconnect():
    """Logs users in via there Facebook account."""
    # Verify login session has been established.
    ...
    # The token stored in the login_session to properly logout.
...
def fbdisconnect():
    """Invalidates Facebook access token.

    This method deletes the Facebook access token therefore
    invalidating it.
    """
...
def item_JSON(category_name, item_name):
    """Displays item information in JSON format."""
...
def category_items_JSON(category_name):
    """Displays the items in a category in JSON format."""
...
def categories_JSON():
    """Displays categories in JSON format."""
...
def create_category():
    """Creates new category and adds it to the database."""
    ...
    if request.method == "POST":
        # Create category and update database.
        if "create" in request.form:
            new_category = Category(category=request.form["category"],
...
def edit_category(category_name):
    """Edits category in the database."""
    # Check user access rights.
    if "user_id" not in login_session:
        return redirect(url_for("show_login"))
...
def show_item(category_name, item_name):
    """Displays info on a specific item."""
    category = session.query(Category).filter_by(category=category_name).one()
    item = session.query(Item).filter_by(name=item_name).one()
    ...
    if request.method == "POST":
        ...
        if "create" in request.form:
            ...
            category = session.query(Category).filter_by(
                category=request.form["categories"]).one()
...
def create_local_user():
    """Creates a local user and logs them in."""
    if request.method == "POST":
        # Create the user and setup the session.
        if "create" in request.form:
            login_session["provider"] = "local"
...
def get_user_id(email):
    """Gets the user' id from there email.

    Args:
        email: The user's email address to get the user id for.

    Return:
        The user's id or None if it does not exist.
    """
    try:
...
def datetime_formatter(date, format="%b %d %Y"):
    """Formats date output to passed in format.

    Args:
        date: The date to format.
        format: The desired format(DEFAULT ex = Apr 01 2018)
